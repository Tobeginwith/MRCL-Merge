...
from src.trainer import QwenSFTTrainer, QwenSFTOLoRATrainer, QwenSFTRegLoRATrainer
from src.dataset import make_supervised_data_module
from src.params import DataArguments, ModelArguments, TrainingArguments
from train.train_utils import get_peft_state_maybe_zero_3, get_peft_state_non_lora_maybe_zero_3, safe_save_model_for_hf_trainer, count_parameters, freeze_lora_A_matrices, initialize_lora_weights_custom
import pathlib
...
def patch_target_modules(
    model, target_modules: str,lora_namespan_exclude=[]
) -> list[str]:
    linear_cls = torch.nn.modules.Linear
    embedding_cls = torch.nn.modules.Embedding
    forbidden_modules = set()
    vision_model_keys=["visual.patch_embed", "visual.deepstack_merger_list"]
    projector_key="visual.merger"
    lora_conflict_keys=["patch_embed"]
    forbidden_modules.update(vision_model_keys)
    forbidden_modules.add(projector_key)
    forbidden_modules.update(lora_conflict_keys)
    rank0_print(f"target_modules for LoRA: {target_modules}")
    ...
    rank0_print(f"Target modules for LoRA: {module_names}")
...
def train():
    ...
    if training_args.use_olora and training_args.use_reglora:
        raise ValueError("`use_olora` and `use_reglora` cannot be both True. Please choose one of them.")
    ...
    if training_args.lora_enable:
        ...
        target_modules=patch_target_modules(
            model, training_args.lora_target if training_args.lora_target != "all" else [],lora_namespan_exclude=lora_namespan_exclude
        )
        peft_config = LoraConfig(
            r=training_args.lora_rank,
            lora_alpha=training_args.lora_alpha,
            target_modules=target_modules if training_args.lora_target != "all" else find_target_linear_names(model, lora_namespan_exclude=lora_namespan_exclude, num_lora_modules=training_args.num_lora_modules),
    ...
    processor = AutoProcessor.from_pretrained(model_args.model_id, fix_mistral_regex=True)
    ...
    if training_args.use_olora:
        trainer = QwenSFTOLoRATrainer(
            ...
            args=training_args,
            **data_module,
            config_files=training_args.config_files,
            olora_lamda_1=training_args.olora_lamda_1,
            olora_lamda_2=training_args.olora_lamda_2,
        )
    elif training_args.use_reglora:
        trainer = QwenSFTRegLoRATrainer(
            model=model,
            processing_class=processor,
            args=training_args,
            **data_module,
            config_files=training_args.config_files,
            reg_lamda=training_args.reg_lamda,
